...
Module ModGrup
    Public Class GrupModel
        ...
        Public Property NamaAgensi As String
    ...
    Public Function TambahGrup(grup As GrupModel) As Integer
        Dim sql As String =
            "INSERT INTO tbl_grup (nama_grup, agensi_id, genre, negara, tahun_debut, jml_member, lagu_populer)
             VALUES (@namaGrup, @agensiID, @genre, @negara, @tahunDebut, @jmlMember, @laguPopuler)"

        Using conn As MySqlConnection = ModKoneksi.BukaKoneksi()
            Using cmd As New MySqlCommand(sql, conn)
                cmd.Parameters.AddWithValue("@namaGrup", grup.NamaGrup)
                cmd.Parameters.AddWithValue("@agensiID", grup.AgensiID)
    ...
    Public Function GetSemuaGrup() As DataTable
        Dim sql As String =
            "SELECT  g.id            AS 'ID',
                     g.nama_grup     AS 'Nama Grup',
                     a.nama          AS 'Agensi',
                     g.genre         AS 'Genre',
                     g.negara        AS 'Negara Asal',
                     g.tahun_debut   AS 'Tahun Debut',
                     g.jml_member    AS 'Jml. Member',
                     g.lagu_populer  AS 'Lagu Populer'
             FROM    tbl_grup g
             JOIN    tbl_agensi a ON a.id = g.agensi_id
    ...
    Public Function GetGrupByID(id As Integer) As GrupModel
        Dim sql As String =
            "SELECT g.*, a.nama AS nama_agensi
             FROM   tbl_grup g
             JOIN   tbl_agensi a ON a.id = g.agensi_id
             WHERE  g.id = @id
             LIMIT  1"
                ...
                Using reader As MySqlDataReader = cmd.ExecuteReader()
                    If reader.Read() Then
                        Return New GrupModel With {
                            ...
                            .AgensiID = reader.GetInt32("agensi_id"),
                            .NamaAgensi = reader.GetString("nama_agensi"),
    ...
    Public Function CariGrup(keyword As String, filter As FilterKolom) As DataTable
        If String.IsNullOrWhiteSpace(keyword) Then Return GetSemuaGrup()
    ...
    Public Function GetSummary() As Tuple(Of Integer, Integer)
        Dim sql As String =
            "SELECT COUNT(*) AS total_grup, COALESCE(SUM(jml_member), 0) AS total_member
             FROM tbl_grup"
    ...
    Private Function BuildKondisiSearch(filter As FilterKolom) As String
        Select Case filter
            Case FilterKolom.NamaGrup : Return "g.nama_grup  LIKE @keyword"
            Case FilterKolom.Agensi : Return "a.nama       LIKE @keyword"
            Case FilterKolom.Genre : Return "g.genre      LIKE @keyword"
            Case FilterKolom.Negara : Return "g.negara     LIKE @keyword"
            Case Else
                Return "(g.nama_grup LIKE @k1 OR a.nama LIKE @k2 OR g.genre LIKE @k3 OR g.negara LIKE @k4)"
        End Select
    End Function
